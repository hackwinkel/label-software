...
*
* Controls the LEDs on the "label"-badge.
*
* antisocial version - continuously transmits a 38kHz IR signal to blind all
* other badges, and not listening for any pulses
*
* Uses a timer T16 interrupt for delays.
* Uses timer T2 to generate a 38Khz IR modulation signal
*/
...
/*******************************************************************************
* as an antisocial badge we want to continuously transmit a 38kHz signal
* using the IR LED connected on PA3 using timer 2. This only needs to be set-up
* once and can be left running - eating batteries
...
void setup() { // Arduino-like setup function called once after power on
	...
	spamIR(); // be antisocial and start spamming a 38kHz IR signal continuously
...
void loop()
{
	...
	switch (state)
	{
		case 1:	twoledsflap(4); break;
		case 2:	twoledsccw(8); break;
		case 3:	twoledscw(8); break;
		case 4:	singleledccw(4); break;
		case 5:	twoledsrandom(4); break;
		case 6:	singleledcw(4); break;
		case 7:	twoledsflapdown(8); break;
		case 8:	twoledsflapup(8); break;
		default: state=0; // will immediately be incremented
		break;
	}
}
